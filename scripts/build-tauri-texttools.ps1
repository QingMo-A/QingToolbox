...
if (-not (Test-Path -LiteralPath $cargoPath -PathType Leaf)) { throw 'cargo was not found. Install Rust stable before building Text Tools.' }
if (-not (Get-Command npm -ErrorAction SilentlyContinue)) { throw 'npm was not found. Install Node.js before building Text Tools.' }

Push-Location $uiRoot
try {
    npm ci --ignore-scripts --no-audit --no-fund
    if ($LASTEXITCODE -ne 0) { throw "Text Tools UI dependency install failed with exit code $LASTEXITCODE" }
    npm run build
    if ($LASTEXITCODE -ne 0) { throw "Text Tools UI build failed with exit code $LASTEXITCODE" }
} finally { Pop-Location }
& $cargoPath build --manifest-path (Join-Path $moduleRoot 'Cargo.toml') --release --locked
if ($LASTEXITCODE -ne 0) { throw "Text Tools Rust module build failed with exit code $LASTEXITCODE" }
$binary = Join-Path $moduleRoot 'target/release/qing-texttools-module.exe'
$uiDist = Join-Path $uiRoot 'dist'
if (-not (Test-Path -LiteralPath $binary -PathType Leaf)) { throw "Text Tools binary was not produced: $binary" }
if (-not (Test-Path -LiteralPath (Join-Path $uiDist 'index.html') -PathType Leaf)) { throw "Text Tools UI was not produced: $uiDist" }

$resolvedResource = [IO.Path]::GetFullPath($resourceRoot)
$expectedSuffix = [IO.Path]::GetFullPath((Join-Path $appRoot 'src-tauri/resources/modules/qing.texttools'))
if ($resolvedResource -ne $expectedSuffix) { throw "Refusing to write outside the Text Tools resource root: $resolvedResource" }
...
Copy-Item -LiteralPath $binary -Destination (Join-Path $resolvedResource 'bin/qing-texttools-module.exe') -Force
...
Write-Host "Text Tools module installed at $resolvedResource"
