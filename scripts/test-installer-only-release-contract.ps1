...
$expectedVersion = '0.2.8-alpha'
if ($metadata.Version -ne $expectedVersion -or $metadata.FileVersion -ne '0.2.8.0') {
    throw "Unexpected 0.2.8 candidate metadata: $($metadata.Version) / $($metadata.FileVersion)"
}
...
if ($workflow -notmatch [regex]::Escape('-Tag v0.2.7-alpha') -or
    $workflow -match [regex]::Escape('-Tag v0.2.5-alpha')) {
    throw 'Preview validation must use the published v0.2.7-alpha upgrade baseline.'
}
...
if ($resolver -notmatch '\[string\]\$Tag\s*=\s*"v0\.2\.7-alpha"' -or
    $candidateGate -notmatch [regex]::Escape('-Tag "v0.2.7-alpha"') -or
    $candidateGate -match [regex]::Escape('-Tag "v0.2.5-alpha"')) {
    throw 'Release candidate scripts do not consistently use the published v0.2.7-alpha baseline.'
}
...
if ($installerScript -notmatch [regex]::Escape('installer\baselines\0.2.7-alpha-host-payload.json') -or
    $installerScript -match [regex]::Escape('installer\baselines\0.2.5-alpha-host-payload.json')) {
    throw 'The installer must clean obsolete host files against the published v0.2.7-alpha payload baseline.'
}
...
$previousCleanupBaseline = Get-Content -LiteralPath (
    Join-Path $repoRoot 'installer\baselines\0.2.7-alpha-host-payload.json') -Raw
foreach ($obsoletePath in @(
    'docs/releases/0.2.7-alpha.md',
    ...
    if ($previousCleanupBaseline -notmatch [regex]::Escape($obsoletePath)) {
        throw "Published v0.2.7-alpha cleanup baseline is missing: $obsoletePath"
    }
}
...
$releaseNotes = Get-Content -LiteralPath (Join-Path $repoRoot 'docs\releases\0.2.8-alpha.md') -Raw -Encoding UTF8
...
if ($releaseNotes -notmatch [regex]::Escape('v0.2.7-alpha')) {
