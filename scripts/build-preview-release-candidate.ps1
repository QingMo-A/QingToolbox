...
try {
    ...
    Write-Host "`n==> Resolve verified v0.2.7-alpha installer"
    $previousDirectory = Join-Path $tempRoot "PreviousInstaller"
    $previous = Invoke-CheckedStage -StageName "Resolve v0.2.7-alpha installer" -Action {
        & (Join-Path $PSScriptRoot "resolve-previous-preview-installer.ps1") `
            -Tag "v0.2.7-alpha" -InstallerPath $PreviousInstallerPath -OutputDirectory $previousDirectory
    }
    if ($null -eq $previous -or -not (Test-Path -LiteralPath $previous.InstallerPath -PathType Leaf)) {
        throw "0.2.8-alpha RC is blocked: verified v0.2.7-alpha installer is unavailable."
    }

    Write-Host "`n==> Test v0.2.7-alpha to 0.2.8-alpha upgrade"
    ...
    Write-Host "Previous version:  0.2.7-alpha"
