...
try {
    ...
    Write-Host "`n==> Resolve verified v0.1.0-alpha installer"
    $previousDirectory = Join-Path $tempRoot "PreviousInstaller"
    $previous = Invoke-CheckedStage -StageName "Resolve v0.1.0-alpha installer" -Action {
    ...
    if ($null -eq $previous -or -not (Test-Path -LiteralPath $previous.InstallerPath -PathType Leaf)) {
        throw "0.2.1-alpha RC is blocked: verified v0.1.0-alpha installer is unavailable."
    }

    Write-Host "`n==> Test v0.1.0-alpha to 0.2.1-alpha upgrade"
