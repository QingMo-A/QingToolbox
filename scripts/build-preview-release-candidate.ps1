...
try {
    ...
    Write-Host "`n==> Resolve verified 0.2.0-alpha installer"
    $previousDirectory = Join-Path $tempRoot "PreviousInstaller"
    $previous = Invoke-CheckedStage -StageName "Resolve 0.2.0-alpha installer" -Action {
        & (Join-Path $PSScriptRoot "resolve-previous-preview-installer.ps1") `
            -Tag "v0.2.0-alpha" -InstallerPath $PreviousInstallerPath -OutputDirectory $previousDirectory
    }
    if ($null -eq $previous -or -not (Test-Path -LiteralPath $previous.InstallerPath -PathType Leaf)) {
        throw "0.2.1-alpha RC is blocked: verified 0.2.0-alpha installer is unavailable."
    }

    Write-Host "`n==> Test 0.2.0-alpha to 0.2.1-alpha upgrade"
    ...
    Write-Host "Previous version:  0.2.0-alpha"
