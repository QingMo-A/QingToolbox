...
try {
  ...
  if($probe.probeId-ne$probeId-or$probe.protocolVersion-ne 3-or-not$probe.navigationSucceeded-or-not$probe.readyIdentitySucceeded-or-not$probe.snapshotValidated-or-not$probe.activationPingSucceeded-or-not$probe.workspaceActivated-or$probe.usedMockTransport){throw "Development Web Shell probe failed; failureCode=$($probe.failureCode); navigation=$($probe.navigationSucceeded); readyIdentity=$($probe.readyIdentitySucceeded); snapshotValidated=$($probe.snapshotValidated); activationPing=$($probe.activationPingSucceeded); workspaceActivated=$($probe.workspaceActivated); mock=$($probe.usedMockTransport)."}
