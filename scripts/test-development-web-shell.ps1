...
try {
  ...
  if($probe.probeId-ne$probeId-or$probe.protocolVersion-ne 2-or-not$probe.navigationSucceeded-or-not$probe.readySucceeded-or-not$probe.snapshotSucceeded-or-not$probe.pingSucceeded-or$probe.usedMockTransport){throw "Development Web Shell probe failed; failureCode=$($probe.failureCode); navigation=$($probe.navigationSucceeded); ready=$($probe.readySucceeded); snapshot=$($probe.snapshotSucceeded); ping=$($probe.pingSucceeded); mock=$($probe.usedMockTransport)."}
