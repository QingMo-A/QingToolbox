...
try {
  ...
  if($process){$process.Refresh();if(-not$process.HasExited){$previous=$ErrorActionPreference;$ErrorActionPreference='SilentlyContinue';& taskkill.exe /PID $process.Id /T /F 2>$null|Out-Null;$ErrorActionPreference=$previous;$global:LASTEXITCODE=0}}
