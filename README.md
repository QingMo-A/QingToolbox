...
- B1 事务所有权边界已加固：Marker 保留到 `Committed` 原子落盘之后，提交后清理失败绝不回滚；Verified Staging 到 candidate 使用稳定句柄复制，Journal 按物理根/环境/模块隔离，并通过四个真实子进程崩溃窗口验证。
...
- B1 的安全关键目录替换使用源目录句柄执行 Win32 `FileRenameInfo`，不再通过路径 `Directory.Move`；双遍精确树快照会拒绝验证期间的结构变化。未提交回滚会先静默并卸载新版 Runtime，再恢复旧目录和旧版运行意图，确保磁盘与运行版本一致。
