...
安装器会创建开始菜单快捷方式；桌面快捷方式默认不勾选。安装包是
self-contained，因此不要求预先安装 .NET 10 Desktop Runtime。安装器当前
没有代码签名，Windows SmartScreen 可能显示未知发布者警告。

卸载可使用“Windows 设置 → 应用 → 已安装的应用 → QingToolbox → 卸载”，
或开始菜单中的“卸载 QingToolbox / Uninstall QingToolbox”。卸载默认保留
