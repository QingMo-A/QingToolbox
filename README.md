...
- 对已下载并校验的官方 `.qmod` 执行稳定句柄离线验证，并原子发布到环境隔离的 Verified Staging；完整 Release 身份控制共享，崩溃可恢复的跨进程锁保证 module/version 唯一发布，失败提交会隔离而不会毒化 Verified。暂存不等于安装，不接触用户模块目录，也不加载 DLL。
