...
当前 Alpha：**0.2.3-alpha**（发布 Tag：`v0.2.3-alpha`）。该版本默认使用经过验证的 Vue 3 工作区，并保留原生 WPF 安全回退；同时让 Vue 外观模式同步到原生标题栏，并延续宿主安全更新、
真正的最近使用模块和一键启动体验，但仍不是正式稳定版，也不代表生产环境可用。
正式下载仅提供 `QingToolbox-0.2.3-alpha-win-x64-setup.exe` 及其同名 SHA256；安装器
...
当前发布使用 `QingToolbox-0.2.3-alpha-win-x64-setup.exe`。安装器基于 Inno Setup，
...
宿主 `0.2.3-alpha` Release 均不捆绑模块 DLL 或 `.qmod`；这些模块将由项目所有者通过独立模块
...
`.qmod` 本质是 ZIP 模块包。`0.2.3-alpha` 尚未实现包签名；模块加载后拥有
...
- 0.2.3-alpha 发布说明：[`docs/releases/0.2.3-alpha.md`](docs/releases/0.2.3-alpha.md)
