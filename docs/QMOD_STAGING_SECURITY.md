...
The execution environment is bound when the staging service is constructed; callers cannot select it per request. Unsafe roots, overlapping roots, and unsupported environments have structured configuration failures. The local asynchronous gate includes canonical staging root, environment, module and version. Cross-process ownership uses a persistent, empty-or-owner-marked lock host opened through Win32 with `FILE_FLAG_OPEN_REPARSE_POINT`, read/write access and no sharing. The file name is a stable hash of the same logical identity. The file handle, not the marker bytes, owns the lock. The OS closes the exclusive handle when a process crashes; the next process can reopen it, observe the non-sensitive stale PID marker, record crash recovery, and clear it on normal release. Marker clearing is best-effort diagnostic cleanup: failure emits a path-free warning while the OS handle and local gate are still released. Tests launch independent worker processes, kill one while it owns the handle, and verify recovery without deleting the lock file.
