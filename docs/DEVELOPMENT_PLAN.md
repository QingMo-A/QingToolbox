...
不再扩展 B1 内核。核心仍只允许 `Development`/`ModuleTest` 执行；Production UI、Production
生命周期适配器和真实 TextTools 金丝雀尚未接入。Preview 2 未执行的人工验收继续为 `Not Run`。
...
下一步为 B2：Production 生命周期适配器与 Development-only TextTools 金丝雀。本轮没有开始 B2、
宿主自更新、Production 安装或真实模块替换。
