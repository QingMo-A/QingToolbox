...
当前实现新增严格的根级 `qmod.json` schema 1，将已下载并验证的官方 Release 身份与 `module.json` 绑定；执行 ZIP Entry、特殊类型、ZIP Bomb、Manifest 和 TOCTOU 复验后，只原子发布到环境隔离的 Verified Staging。共享任务要求完整包身份一致，而非仅 SHA 相同；环境由服务绑定，同 module/version 的并发和跨 Service 发布只允许一个 SHA。复用前会严格复验全部元数据和不跟随 Reparse 的完整目录树，发现篡改只返回结构化失败，不自动删除或覆盖。Staging 不等于安装，不接触正式模块目录，也不加载或执行 DLL。安全边界见 `docs/QMOD_STAGING_SECURITY.md`。
