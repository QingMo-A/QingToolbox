...
当前阶段 A 已使用稳定包句柄和完整官方 Release 身份约束共享；本地与跨进程发布锁按 root/environment/module/version 隔离，独立 Worker 已覆盖竞争、持锁崩溃恢复和取消。等待发布锁不占用解压容量。Move 后只有最终 attestation 通过才记录 Verified 成功；当前事务失败目录按 transaction identity 隔离到 Rejected。复用树通过不允许删除的目录句柄和同一文件句柄完成路径、长度与 Hash 复验。Staging 仍不等于安装，不接触正式模块目录，也不加载或执行 DLL。安全边界见 `docs/QMOD_STAGING_SECURITY.md`。
