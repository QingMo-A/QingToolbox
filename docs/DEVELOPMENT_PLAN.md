...
当前实现新增严格的根级 `qmod.json` schema 1，将已下载并验证的官方 Release 身份与 `module.json` 绑定；执行 ZIP Entry、特殊类型、ZIP Bomb、Manifest 和 TOCTOU 复验后，只原子发布到环境隔离的 Verified Staging。Staging 不等于安装，不接触正式模块目录，也不加载或执行 DLL。安全边界见 `docs/QMOD_STAGING_SECURITY.md`。
