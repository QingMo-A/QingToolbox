...
阶段 A 已达到 **Engineering Complete**：稳定包句柄和完整官方 Release 身份约束共享；本地与跨进程发布锁按 root/environment/module/version 隔离，独立 Worker 覆盖竞争、持锁崩溃恢复和取消，且锁等待不占解压容量。Incoming candidate 在原子移动前完成 metadata、Manifest、路径、长度与 Hash 的稳定句柄认证，`Directory.Move` 是唯一 Verified 发布提交点；移动后的取消、日志及锁 marker 诊断清理不能改写成功。未来复用仍执行同等严格认证。Staging 仍不等于安装，不接触正式模块目录，也不加载或执行 DLL。阶段 B 尚未开始，Preview 2 剩余人工发布验收继续为 `Not Run`。安全边界见 `docs/QMOD_STAGING_SECURITY.md`。
