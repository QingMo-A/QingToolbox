...
阶段 A 已达到 **Engineering Complete**：稳定包句柄和完整官方 Release 身份约束共享；本地与跨进程发布锁按物理 root/environment/module/version 隔离，独立 Worker 覆盖竞争、持锁崩溃恢复和取消，且锁等待不占解压容量。Incoming candidate 在原子移动前完成 metadata、Manifest、路径、长度与 Hash 的稳定句柄认证，`Directory.Move` 与 committed 标记位于同一线性化提交区；Caller 取消只能发生在提交前或提交后，不能观察到已 Move 但未 committed 的状态。移动后的取消、日志及锁 marker 诊断清理不能改写成功，未来复用仍执行同等严格认证。Staging/UserModules 配置根要求绝对、非卷根、物理不重叠且无 Reparse。Staging 仍不等于安装，不接触正式模块目录，也不加载或执行 DLL。阶段 B 尚未开始，Preview 2 剩余人工发布验收继续为 `Not Run`。安全边界见 `docs/QMOD_STAGING_SECURITY.md`。

### 8.2 阶段 B：0.3.0-alpha 模块事务更新

阶段 B 已开始。B1 的可恢复事务安装核心已达到 **Engineering Complete**：可信 Verified Staging
重新认证、持久化严格 Journal、物理根/environment/module 跨进程锁、同卷 candidate、旧目录备份、
原子 promotion、静态安装复验、失败回滚与崩溃恢复均已由独立 smoke test 覆盖。核心当前只允许
`Development`/`ModuleTest` 执行；Production UI、Production 生命周期适配器和真实 TextTools 金丝雀
更新尚未接入。Preview 2 尚未执行的人工验收项目继续为 `Not Run`。边界见
`docs/MODULE_UPDATE_TRANSACTION.md`。

B1 的提交与所有权边界现已封闭：事务 Marker 和 backup 保留到 `Committed` Journal 原子落盘，
此后任何 Marker、work 或 Journal 清理异常只能进入可恢复的 `CleanupPending`，绝不回滚新版。
Staging→candidate 使用同一稳定句柄完成物理边界认证、Hash 与 Copy；阶段 A/B 的跨进程锁复用
崩溃可恢复独占 Windows 句柄。Journal 按物理根/environment/module 隔离，安全清理不跟随
...
B1 原子替换边界现已封闭：installed→backup、candidate→installed、installed→failed-candidate 和
backup→installed 全部通过已认证源目录句柄调用 Win32 `FileRenameInfo`，并持有目标父目录 Lease，
不再存在路径检查与 `Directory.Move` 之间的可观察替换窗口，也没有不安全降级。树认证使用包含对象
File ID、长度和 SHA256 的双遍快照；Lock 与 Journal 子文件操作在稳定父目录 Lease 中完成。若新版
Runtime 已恢复但 Commit 尚未落盘，回滚会先静默并卸载新版，再恢复旧目录并重新应用旧版运行意图。
五个真实子进程窗口现包含 candidate copy 中途崩溃。B1 继续保持 **Engineering Complete**；下一步
仍是 B2 Production 生命周期适配器与 Development-only TextTools 金丝雀，Preview 2 未执行人工验收
继续为 `Not Run`。
