# QingToolbox 0.2.8-alpha Acceptance Checklist
...
| Notes | Candidate is installer-only and not published. The published upgrade baseline is `v0.2.7-alpha`. |
...
## A. v0.2.7-alpha → 0.2.8-alpha in-place upgrade
...
Automated coverage installs v0.2.7-alpha into a custom path containing spaces, then runs 0.2.8-alpha and Repair without `/DIR`. It verifies automatic directory discovery, old/new PID replacement, single-instance restart, and absence of a second default-directory installation. These automated checks do not change the manual statuses above.
...
| Check | Status | Evidence / notes |
|---|---|---|
| Run the same 0.2.8-alpha installer again | Not Run | Local repair was not run; exact-HEAD CI is pending |
...
| Check | Status | Evidence / notes |
|---|---|---|
| Uninstall 0.2.8-alpha and confirm host files are removed | Not Run | |
