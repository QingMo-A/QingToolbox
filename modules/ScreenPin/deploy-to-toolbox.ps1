...
$target=Join-Path $hostRoot "QingToolbox.Shell\bin\$Configuration\net10.0-windows\Modules\ScreenPin"
New-Item -ItemType Directory -Force $target|Out-Null
foreach($file in @("QingToolbox.Modules.ScreenPin.dll","QingToolbox.Modules.ScreenPin.deps.json","module.json","icon.svg")){$source=Join-Path $output $file;if(Test-Path $source){Copy-Item $source $target -Force}}
Copy-Item (Join-Path $PSScriptRoot "i18n") $target -Recurse -Force
Write-Host "Screen Pin deployed to: $target"
