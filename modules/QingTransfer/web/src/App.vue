...
<template>
  <main class="shell">
    ...
    <div v-if="state.incomingConnection" class="modal-backdrop"><div class="modal" role="dialog" aria-modal="true" aria-labelledby="incoming-connection-title"><div class="modal-icon"><QingIcon name="phone" /></div><span class="section-label">{{ t('status.incoming', 'Incoming request') }}</span><h2 id="incoming-connection-title">{{ state.incomingConnection.name }}</h2><p>{{ t('status.incoming', 'Incoming request from {0}').replace('{0}', state.incomingConnection.name) }}</p><div class="modal-actions"><button class="primary" :disabled="busy" @click="run('acceptIncomingConnection')">{{ t('actions.accept', 'Accept') }}</button><button class="secondary" :disabled="busy" @click="run('rejectIncomingConnection')">{{ t('actions.reject', 'Reject') }}</button></div></div></div>
    <div v-if="state.incomingFile" class="modal-backdrop"><div class="modal" role="dialog" aria-modal="true" aria-labelledby="incoming-file-title"><div class="modal-icon"><QingIcon name="folder" /></div><span class="section-label">{{ t('receive.incoming', 'Incoming file') }}</span><h2 id="incoming-file-title">{{ state.incomingFile.name }}</h2><span class="file-badge">{{ formatFileSize(state.incomingFile.size) }}</span><p>{{ t('receive.incomingHint', 'Choose where to save this file.') }}</p><div class="modal-actions"><button class="primary" :disabled="fileDecisionBusy" @click="run('acceptIncomingFile')">{{ t('actions.choose', 'Choose') }}</button><button class="secondary" :disabled="fileDecisionBusy" @click="run('rejectIncomingFile')">{{ t('actions.reject', 'Reject') }}</button></div></div></div>
