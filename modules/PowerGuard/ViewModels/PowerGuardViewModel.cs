...
public sealed class PowerGuardViewModel:INotifyPropertyChanged,IDisposable
{
    ...
    private void RefreshComputed(){foreach(var n in new[]{nameof(GuardStatusText),nameof(NetworkStatusText),nameof(StateText),nameof(Countdown),nameof(CountdownVisibility),nameof(LastOnlineText),nameof(DefaultActionText),nameof(CanSave),nameof(CanProbe),nameof(CanTest),nameof(CanSuppress),nameof(CanRearm)})Changed(n);}
