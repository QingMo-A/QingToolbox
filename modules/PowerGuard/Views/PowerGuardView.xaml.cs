...
public partial class PowerGuardView : UserControl, ILocalizedModuleView, IDisposable
{
    ...
    private async void Cancel_Click(object s,RoutedEventArgs e){try{if(await _viewModel.SuppressAsync()==GuardOperationResult.NotAvailable)StatusText.Text=T("status.operationUnavailable","This operation is not available in the current state.");}catch{StatusText.Text=T("errors.operationFailed","Operation failed.");}}
    private async void Rearm_Click(object s,RoutedEventArgs e){try{if(await _viewModel.RearmAsync()==GuardOperationResult.NotAvailable)StatusText.Text=T("status.operationUnavailable","This operation is not available in the current state.");}catch{StatusText.Text=T("errors.operationFailed","Operation failed.");}}
