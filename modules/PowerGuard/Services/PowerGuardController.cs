...
{
    Task ShowAsync(bool testMode, int seconds, CancellationToken token = default);
    Task UpdateAsync(int seconds, bool recovering, CancellationToken token = default);
...
public sealed class PowerGuardController : IAsyncDisposable
{
    ...
    public PowerGuardSettings Settings { get; private set; }
    ...
    public async Task ActivateAsync(CancellationToken token = default)
    {
        ...
        try
        {
            ...
            if (Settings.GuardEnabled) Subscribe();
    ...
    public async Task DeactivateAsync(CancellationToken token = default)
    {
        ...
        try
        {
            ...
            Unsubscribe();
            await _warning.CloseAsync(token);
    ...
    private async Task MonitorAsync(CancellationToken token)
    {
        ...
        _machine.MoveTo(Settings.GuardEnabled ? PowerGuardState.StartupGrace : PowerGuardState.Disabled, now);
        Publish(false);
    ...
    internal async Task ProcessProbeAsync(ConnectivityProbeResult result, DateTimeOffset now, CancellationToken token = default)
    {
        if (result.IsOnline)
        {
            ...
            if (_machine.State is PowerGuardState.Countdown or PowerGuardState.SuspectedOffline or PowerGuardState.ActionFailed)
            {
                ...
                _machine.MoveTo(PowerGuardState.Recovering, now); await _warning.CloseAsync(token);
            }
            ...
            {
                Settings.SuppressedUntilConnectivityRestored = false;
                await _settingsStore.WriteAsync(Settings, token);
        ...
        _recoverySince = null;
        if (Settings.SuppressedUntilConnectivityRestored || _machine.State == PowerGuardState.SuppressedForCurrentOutage)
        {
            _machine.MoveTo(PowerGuardState.SuppressedForCurrentOutage, now); Publish(false); return;
        }
        if (_machine.State == PowerGuardState.StartupGrace && Elapsed(now) < Settings.StartupGraceSeconds) { Publish(false); return; }
        ...
        if (_machine.State == PowerGuardState.SuspectedOffline && _offlineSince is { } offline &&
            SecondsBetween(offline, now) >= Settings.OfflineConfirmationSeconds)
        {
            _countdownDeadline = now.AddSeconds(Settings.ShutdownCountdownSeconds);
            _machine.MoveTo(PowerGuardState.Countdown, now);
            await _warning.ShowAsync(false, Settings.ShutdownCountdownSeconds, token);
        ...
        if (_machine.State == PowerGuardState.Countdown && _countdownDeadline is { } deadline)
        {
            ...
            await _warning.UpdateAsync(remaining, false, token);
            if (remaining == 0) await ExecuteAfterFinalProbeAsync(token);
        }
        Publish(false);
    }
    ...
    private async Task ExecuteAfterFinalProbeAsync(CancellationToken token)
    {
        ...
        if (final.IsOnline)
        {
            ...
            _countdownDeadline = null; await _warning.CloseAsync(token); Publish(true); return;
        }
        ...
        if (!await _power.RequestNormalShutdownAsync(token))
        {
    ...
    {
        Settings.SuppressedUntilConnectivityRestored = true;
        await _settingsStore.WriteAsync(Settings, token);
        _countdownDeadline = null; _machine.MoveTo(PowerGuardState.SuppressedForCurrentOutage, DateTimeOffset.UtcNow);
        await _warning.CloseAsync(token); await _events.AppendAsync("OutageSuppressed", token: token); Publish(false);
    }

    public async Task RearmAsync(CancellationToken token = default)
    {
        Settings.SuppressedUntilConnectivityRestored = false; await _settingsStore.WriteAsync(Settings, token);
    ...
    {
        if (_countdownDeadline is null) return;
    ...
    public Task ShowTestWarningAsync(CancellationToken token = default) => _warning.ShowAsync(true, 30, token);
    ...
    {
        settings.Normalize(); await _settingsStore.WriteAsync(settings, token); Settings = settings;
        if (!settings.GuardEnabled)
        {
            Unsubscribe();
            _countdownDeadline = null; _offlineSince = null; _recoverySince = null;
            _machine.MoveTo(PowerGuardState.Disabled, DateTimeOffset.UtcNow); await _warning.CloseAsync(token);
        }
    ...
    private void OnPowerModeChanged(object sender, PowerModeChangedEventArgs e)
    {
        ...
        _offlineSince = null; _recoverySince = null; _countdownDeadline = null;
        _machine.MoveTo(PowerGuardState.StartupGrace, DateTimeOffset.UtcNow);
        Observe(_warning.CloseAsync(), "resume warning cleanup");
        Observe(_events.AppendAsync("ResumeGraceStarted"), "resume event write");
    ...
    private PowerGuardSnapshot CreateSnapshot(bool online, string? error = null) => new(_machine.State, Settings.GuardEnabled, online, _lastOnline,
        _countdownDeadline is null ? 0 : Math.Max(0, (int)Math.Ceiling((_countdownDeadline.Value - DateTimeOffset.UtcNow).TotalSeconds)), Settings.SuppressedUntilConnectivityRestored, error);
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        await DeactivateAsync(); Unsubscribe(); _lifecycle.Dispose(); _probeGate.Dispose(); _signal.Dispose();
    }
}
