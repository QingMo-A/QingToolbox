...
using Models = QingToolbox.Modules.PowerGuard.Models;
...
public sealed class WarningWindowPresenter(ILocalizationService localization,string moduleId):IWarningPresenter,IAsyncDisposable
{
    private readonly SemaphoreSlim _operations=new(1,1);private ShutdownWarningWindow? _real,_test;private Func<Task<Models.GuardOperationResult>>? _suppress,_extend,_shutdown;private bool _disposed;
    public void Configure(Func<Task<Models.GuardOperationResult>>suppress,Func<Task<Models.GuardOperationResult>>extend,Func<Task<Models.GuardOperationResult>>shutdown){_suppress=suppress;_extend=extend;_shutdown=shutdown;}
    public async Task ShowRealAsync(int seconds,CancellationToken token=default)=>await SerializedAsync(async()=>await DispatchAsync(()=>{CloseTestCore();if(_real is not null){_real.SetSeconds(seconds);_real.Activate();return;}var window=new ShutdownWarningWindow(localization,moduleId,new(){Seconds=seconds},()=>_suppress?.Invoke()??Task.FromResult(Models.GuardOperationResult.Failed),()=>_extend?.Invoke()??Task.FromResult(Models.GuardOperationResult.Failed),()=>_shutdown?.Invoke()??Task.FromResult(Models.GuardOperationResult.Failed));_real=window;window.Closed+=(_,_)=>{if(ReferenceEquals(_real,window))_real=null;};window.Show();window.PositionAtPrimaryWorkArea();},token),token);
    public async Task<TestPreviewResult> ShowTestAsync(int seconds,CancellationToken token=default)
    { TestPreviewResult result=TestPreviewResult.Opened;await SerializedAsync(async()=>await DispatchAsync(()=>{if(_real is not null){result=TestPreviewResult.UnavailableDuringCountdown;return;}if(_test is not null){_test.Activate();result=TestPreviewResult.ActivatedExisting;return;}var window=new ShutdownWarningWindow(localization,moduleId,new(){IsTestMode=true,Seconds=seconds},async()=>{await CloseTestAsync();return Models.GuardOperationResult.Succeeded;},()=>Task.FromResult(Models.GuardOperationResult.Succeeded),async()=>{await CloseTestAsync();return Models.GuardOperationResult.Succeeded;});_test=window;window.Closed+=(_,_)=>{if(ReferenceEquals(_test,window))_test=null;};window.Show();window.PositionAtPrimaryWorkArea();},token),token);return result; }
    ...
    private async Task SerializedAsync(Func<Task> action,CancellationToken token){await _operations.WaitAsync(token);try{if(_disposed)return;await action();}finally{_operations.Release();}}
    ...
    public async ValueTask DisposeAsync(){await _operations.WaitAsync();try{if(_disposed)return;await DispatchAsync(()=>{CloseTestCore();CloseRealCore();},CancellationToken.None);_disposed=true;_suppress=_extend=_shutdown=null;}finally{_operations.Release();}_operations.Dispose();}
}
