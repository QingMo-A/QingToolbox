...
sealed class FakePower:IPowerActionService{public int Count;public bool Result=true;public Task<bool> RequestNormalShutdownAsync(CancellationToken token=default){Count++;return Task.FromResult(Result);}}
sealed class FakeWarning:IWarningPresenter{public int ShowCount,TestShowCount,CloseCount;public Task ShowAsync(bool testMode,int seconds,CancellationToken token=default){if(testMode)TestShowCount++;else ShowCount++;return Task.CompletedTask;}public Task UpdateAsync(int seconds,bool recovering,CancellationToken token=default)=>Task.CompletedTask;public Task CloseAsync(CancellationToken token=default){CloseCount++;return Task.CompletedTask;}}
