...
export class MockTransport implements Transport {
  ...
  private appearancePresetId='qing-default'
  private fontId='Default'
  private readonly fonts=[
    {id:'Default',source:'default' as const,displayName:'Default',familyName:null,resourceUrl:null},
    {id:'system:Segoe UI',source:'system' as const,displayName:'Segoe UI',familyName:'Segoe UI',resourceUrl:null},
  ...
  async request(message:BridgeRequest):Promise<BridgeResponse>{
    ...
    if(message.command==='modules.checkUpdate'||message.command==='modules.downloadUpdate'){
      ...
      if(Object.keys(message.payload).length!==1||typeof message.payload.moduleId!=='string'||!message.payload.moduleId.trim())return this.error(message,'InvalidPayload')
      const index=this.modules.findIndex(item=>item.id===message.payload.moduleId);if(index<0)return this.error(message,'ModuleNotFound')
      const item=this.modules[index];if(item.isBusy||item.isDownloadActive||item.isUpdateCheckBusy)return this.error(message,'ModuleBusy')
      if(message.command==='modules.checkUpdate')this.modules[index]={...item,updateStatus:item.id==='qing.texttools'?'UpdateAvailable':'UpToDate',targetVersion:item.id==='qing.texttools'?'0.2.0':null,releaseNotes:item.id==='qing.texttools'?'Improves text conversion and reliability.':null,canDownloadUpdate:item.id==='qing.texttools'}
      else{if(!item.canDownloadUpdate)return this.error(message,'ModuleUpdateUnavailable');this.modules[index]={...item,downloadStatus:'Verified',isDownloadActive:false,downloadBytesReceived:item.downloadExpectedBytes,canDownloadUpdate:false,canInstallVerifiedUpdate:true}}
      return this.ok(message,this.moduleSnapshot())
    }
    if(message.command==='modules.installVerifiedUpdate'){
      if(this.phase!=='Activated')return this.error(message,'BridgeNotActivated')
      if(Object.keys(message.payload).length!==1||typeof message.payload.moduleId!=='string'||!message.payload.moduleId.trim())return this.error(message,'InvalidPayload')
      const index=this.modules.findIndex(item=>item.id===message.payload.moduleId);if(index<0)return this.error(message,'ModuleNotFound')
      const item=this.modules[index];if(!item.canInstallVerifiedUpdate)return this.error(message,'ModuleUpdateUnavailable')
      const targetVersion=item.targetVersion!;this.modules[index]={...item,version:targetVersion,updateStatus:'NotChecked',targetVersion:null,releaseNotes:null,downloadStatus:'NotDownloaded',downloadBytesReceived:0,downloadExpectedBytes:0,canInstallVerifiedUpdate:false}
      return this.ok(message,{disposition:'Installed',sourceVersion:item.version,targetVersion,snapshot:this.moduleSnapshot()})
    }
    ...
    if(message.command==='settings.setFont'){if(this.phase!=='Activated')return this.error(message,'BridgeNotActivated');const value=message.payload.fontId;if(Object.keys(message.payload).length!==1||typeof value!=='string'||!this.fonts.some(font=>font.id===value))return this.error(message,'InvalidPayload');this.fontId=value;return this.ok(message,this.settingsSnapshot())}
    if(message.command==='settings.importFont'){if(this.phase!=='Activated')return this.error(message,'BridgeNotActivated');if(Object.keys(message.payload).length!==0)return this.error(message,'InvalidPayload');return this.ok(message,{disposition:'Cancelled',snapshot:this.settingsSnapshot()})}
