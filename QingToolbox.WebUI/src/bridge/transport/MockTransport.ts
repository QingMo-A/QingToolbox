...
export class MockTransport implements Transport {
  ...
  async request(message:BridgeRequest):Promise<BridgeResponse>{
    ...
    if(message.command==='settings.getSnapshot'){if(this.phase!=='Activated')return this.error(message,'BridgeNotActivated');if(Object.keys(message.payload).length!==0)return this.error(message,'InvalidPayload');return this.ok(message,{generatedAt:'2026-07-25T12:00:00.000Z',language:{code:'en-US',displayName:'English'},showLogsInSidebar:true,mainWindowCloseBehavior:'MinimizeToNotificationArea',closeBehaviorMessage:'The main window minimizes to the notification area.',launchAtLogin:false,canConfigureLaunchAtLogin:true,startupPresentationMode:'FloatingBadge',startupBackend:'Registry Run',startupStatus:'Healthy',startupMessage:'Windows startup registration is healthy.'})}
