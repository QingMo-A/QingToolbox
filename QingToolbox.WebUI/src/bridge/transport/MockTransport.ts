...
export class MockTransport implements Transport {
  ...
  async request(message:BridgeRequest):Promise<BridgeResponse>{
    ...
    if(message.command==='modules.checkUpdate'||message.command==='modules.downloadUpdate'){
      ...
      else{if(!item.canDownloadUpdate)return this.error(message,'ModuleUpdateUnavailable');this.modules[index]={...item,downloadStatus:'Verified',isDownloadActive:false,downloadBytesReceived:item.downloadExpectedBytes,canDownloadUpdate:false}}
