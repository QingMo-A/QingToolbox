...
async function operate(module: ModuleSnapshotItem, operation: ModuleOperation) {
  ...
  try {
    const snapshot = operation === 'load' ? await client.load(module.id) : await client.activate(module.id)
    store.complete(snapshot)
    toast.show(`${module.displayName} ${operation === 'load' ? 'loaded' : 'activated'}.`, 'success')
  } catch (error) {
...
const operationLabel = (module: ModuleSnapshotItem, operation: ModuleOperation) => store.operations[module.id] === operation ? (operation === 'load' ? 'Loading…' : 'Activating…') : (operation === 'load' ? 'Load' : 'Activate')
