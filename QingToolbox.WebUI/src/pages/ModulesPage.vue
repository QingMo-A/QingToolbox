...
async function operate(module: ModuleSnapshotItem, operation: ModuleOperation) {
  ...
  try {
    const snapshot = operation === 'load' ? await client.load(module.id) : operation === 'activate' ? await client.activate(module.id) : operation === 'open' ? await client.open(module.id) : operation === 'deactivate' ? await client.deactivate(module.id) : await client.unload(module.id)
...
const operationLabel = (module: ModuleSnapshotItem, operation: ModuleOperation) => store.operations[module.id] === operation ? (operation === 'load' ? 'Loading…' : operation === 'activate' ? 'Activating…' : operation === 'open' ? 'Opening…' : operation === 'deactivate' ? 'Deactivating…' : 'Unloading…') : (operation === 'load' ? 'Load' : operation === 'activate' ? 'Activate' : operation === 'open' ? 'Open' : operation === 'deactivate' ? 'Deactivate' : 'Unload')
...
.module-startup>p,.module-startup-status{color:var(--q-text-2);font-size:12px}.module-startup .settings-switch-row{margin-top:8px}.module-startup-status{margin-top:4px}
