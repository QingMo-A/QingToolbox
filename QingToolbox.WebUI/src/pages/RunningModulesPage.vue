...
async function openModule(module: ModuleSnapshotItem) {
  if (!modules.beginOperation(module.id, 'open')) return
  try { modules.complete(await client.open(module.id)); toast.show(`${module.displayName} window opened or focused.`, 'success') }
