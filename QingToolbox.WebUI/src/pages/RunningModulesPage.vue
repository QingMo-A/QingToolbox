    ...
    <section v-else-if="modules.runningModules.length" class="running-module-stack">
      <article v-for="module in modules.runningModules" :key="module.id" class="running-module-card">
        ...
        <div class="running-module-action">
          <QButton v-if="module.canOpen && !module.isExecutionBlocked" variant="primary" :disabled="!!modules.operations[module.id] || module.isBusy" @click="operate(module, 'open')">{{ modules.operations[module.id] === 'open' ? 'Opening…' : 'Open' }}</QButton>
          <QButton v-if="module.canDeactivate && !module.isExecutionBlocked" :disabled="!!modules.operations[module.id] || module.isBusy" @click="operate(module, 'deactivate')">{{ modules.operations[module.id] === 'deactivate' ? 'Deactivating…' : 'Deactivate' }}</QButton>
          <QButton v-if="module.canUnload && !module.isExecutionBlocked" :disabled="!!modules.operations[module.id] || module.isBusy" @click="operate(module, 'unload')">{{ modules.operations[module.id] === 'unload' ? 'Unloading…' : 'Unload' }}</QButton>
