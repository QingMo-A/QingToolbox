...
<template><main class="diagnostics-page">
  ...
  <section class="status" aria-live="polite"><span :class="{ ok: store.bridge === 'Connected' }">{{ store.bridge }}</span><strong>{{ store.mode }} mode</strong><span v-if="store.error">{{ store.error }}</span><div class="theme-switch"><button v-for="mode in themes" :key="mode" :class="{active:theme.mode===mode}" @click="theme.set(mode)">{{mode}}</button></div></section>
